...
## 2. The Threat Model
Shafahiらのpoisoning攻撃のように、我々が想定する攻撃者は、少数の汚染画像を攻撃対象モデルが使用する学習データセットに注入する。攻撃者の目的は、汚染された学習データセットを基に作成された攻撃対象モデルに、（学習データセットに含まれない）テストデータを攻撃者の意図した特定クラスに誤分類させることである。攻撃者が摂動`δ`をベース画像に加えてこの目的を達成する画像分類タスクを考えると、汚染画像`x+δ`がベース画像`x`と同じクラスを共有し、特定の方法でDNNの決定境界を変更できるように作成される。  
...
1番目のアプローチは**転移学習**である。  
転移学習では、CIFARやImageNetなどの公開データセットで学習した**ResNet**の畳み込み層など、（事前学習された後に）Freezeされた特徴抽出器`φ`が利用される。また、重み`W`やバイアス`b`を持つ線形分類器は、別のデータセット`X`の特徴`φ(X)`でファインチューニングされる。この転移学習は、モデル作成時に大規模なデータセットが利用できない産業用アプリケーションでは一般的に利用されている。これまでの転移学習に対するpoisoning攻撃は、KohとLiangらの研究やShafahiらの研究のように、特徴抽出器の内部構造が明らかになっているホワイトボックスでのみ検証されている。  
