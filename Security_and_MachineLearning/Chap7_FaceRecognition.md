...
Neural Networkは**層**（Layer）と**ノード**（Node）から構成される**分類アルゴリズム**であり、**ベクトル化**された入力データ（画像やテキストなど）を**入力層**（Input layer）から受け取り、分類結果を**出力層**（Output layer）から出力します。入力層と出力層の間には1つ以上の**隠れ層**（Hidden layer）が存在し（隠れ層は**中間層**とも呼ばれる）、入力データを適切に分類するための処理を行います（詳細は後述）。  
...
入力層はベクトル化した入力画像を受け取ります。  
ところで、画像のベクトル化とは何でしょうか？画像は**pixel値の集合**であるため、画像の**pixel値を1pixelずつ抽出**して並べていくと、ベクトルとして表現することができます（通常は各pixel値を正規化した上で並べる）。例えば、**128x128pixelのRGB画像**の場合は、**49,152**（=128x128x3）のベクトルとして表現できます。ベクトル化された入力画像（以下、信号）は入力層の各ノードに入力され、ネットワークで連結された**隠れ層の各ノードに伝搬**していきます。  

次に、隠れ層に伝搬された信号は**活性化**された上で、次の層に伝搬していきます。  
...
吹き出しの左部分は、**隠れ層のm番目ノード**にフォーカスし、入力層から伝搬された信号を活性化して次ノードに伝搬する様子を表しています。  
なお、前層から伝搬される信号を「X」、ノード間の**重み**を「W」、各ノードの**バイアス**を「b」で表しています。また、信号を活性化する関数（**活性化関数**）を「h」で表現し、隠れ層から出力される信号を「y」で表しています。  

| 活性化関数（Activation Function）|
|:--------------------------|
| ノードへの「信号入力の総和」を活性化させる方法を決定付ける関数。シグモイド関数、ソフトマックス関数、ReLU関数、恒等関数など、様々な種類が存在する。|

隠れ層のm番目ノードは、入力層から伝搬してきた信号「X10 ~ X1m」と、**入力層と隠れ層のノード間に存在**する重み「W10 ~ W1m」を**積算**して**和**を取り、これにバイアス値「b1m」を足し合わせた値を受け取ります（吹き出しの下にある数式）。そして、この信号を活性化関数「h」で整えた値「y1m」を次のノードに伝搬します。  

次に、吹き出しの右部分は、**出力層の3番目ノード**にフォーカスし、隠れ層から伝搬された信号を活性化して出力している様子を表しています。  
なお、前層から伝搬される信号を「X」、ノード間の**重み**を「W」、各ノードの**バイアス**を「b」で表しています。また、活性化関数を「h」で表現し、出力層から出力される信号を「O」で表しています。  

出力層の3番目ノードは、n番目の隠れ層から伝搬していきた信号「Xn0 ~ Xnm」と、**隠れ層と出力層のノード間に存在**する重み「Wn0 ~ Wnm」を積算して和を取り、これにバイアス値「b2」を足し合わせた値を受け取ります（吹き出しの下にある数式）。そして、この信号を活性化関数「h」で整えた値「O2」を出力します。出力層はNeural Networkの最後の層になるため、この出力値がNeural Networkの「答え」となります。  
