...
例えば「32×32pixel」の白黒画像の場合、入力信号は1024(=32x32)のベクトルとなりますね。このため、**入力画像のレイアウトが少しでも異なると、入力信号のベクトルは大きく異なってしまいます**。  

下図に示す2つの画像は共に筆者（Isao Takaesu）ですが、レイアウトが異なるため、2つを同じ人物として認識することが非常に困難に難しくなります。このように、Neural Networkの「1pixel単位で入力を受け取る」ことが、頑健性の低下の大きな要因となります。  
...
もちろん、世の中に存在するあらゆる筆者の顔画像で学習すれば、多少は認識精度は向上するかもしれませんが、あまりに学習コストが掛かりすぎます。また、新たな顔画像に対しては、上記と同じ理由で認識することが困難となります。  
