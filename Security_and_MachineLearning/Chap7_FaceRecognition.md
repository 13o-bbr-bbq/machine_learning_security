...
このグラフは、とある関数「`f(x)`」の軌跡を表しています（今は関数の中身は気にしてなくても大丈夫です）。  
ところで、このグラフにおける最小値は、見て分かる通り原点「`(0, 0)`」になりますね。とても簡単です。しかし、これは我々がグラフ全体を俯瞰（関数の形状を把握）しているから分かるのであり、Neural Networkの学習では俯瞰することはできません。では、グラフ全体を俯瞰できない条件の下、どのようにして最小値を求めるのでしょうか？そこで勾配降下法の登場です。  

勾配下降法を一言でいうと、「**関数の傾きを利用**して、その関数の最小値を求める方法」となります。  
