...
ここで、出力層の各ノードが保持する値の総和は1になることを上述しましたが、これにより各ノードの値を**分類確率**と見なすことができます。上図では、3番目のノードが保持する値は「0.9378」になっているため、「このNeural Networkは、**93.78%の確率**で入力画像を**Isao Takaesuと判断**した」と言い換えることができます。  

#### 7.3.1.2. Neural Networkのノード
ところで、Neural Networkの各層のノード数は幾つにすれば良いのでしょうか？  
これは、入力されるデータのサイズと分類したいクラス数によって変わります。  

入力層のノード数は入力データのサイズによって変わります。例えば、**32x32pixelの白黒画像**を分類したい場合、受け取る入力データは1024(=32x32)のベクトルとなるため、ノード数は**1024個**にします（RGBの場合は、1024x3=3072個）。一方、出力層のノード数は分類クラス数によって変わります。例えば、入力データを**5クラスに分類**したい場合、ノード数は**5個**にします。なお、隠れ層のノード数は、受け取るデータのサイズや特性に合わせて調整します（パラメータチューニング）。  
