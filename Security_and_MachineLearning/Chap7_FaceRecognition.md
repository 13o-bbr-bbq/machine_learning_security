...
次に、この誤差を最小化するために、**勾配降下法**と呼ばれる手法を用いて**重みの値を最適化**していきます。  
...
なお、勾配「`⊿x`」は関数「`f(x)`」を係数「`x`」で偏微分すると求まります。  
...
以下は、勾配降下法で最適な重みを求めるイメージです。  
 ...
 <br>
  ...
  1. 適当に選んだ点「`A`」における、誤差関数「`L(w)`」に対する重み「`w`」の**勾配**「`⊿w`」を求める。  
  2. 勾配方向に重み「`w`」の値を更新する（`w - ⊿w`）。その結果、点「`A'`」に移動する。  
  3. 点「`A'`」における、誤差関数「`L(w)`」に対する重み「`w`」の勾配「`⊿w'`」を求める。  
  4. 勾配方向に重み「`w`」の値を更新する（`w - ⊿w'`）。その結果、点「`A''`」に移動する。  
  5. 点「`A''`」における、誤差関数「`L(w)`」に対する重み「`w`」の傾き「`⊿w''`」を求める。 
  6. 勾配方向に重み「`w`」の値を更新する（`w - ⊿w''`）。その結果、点「`O`」に移動する。  
  7. 点「`O`」の勾配は**ほぼ0**のため、この地点を**最小値（誤差が最も少ない地点）**と判断し、重み「`w`」の更新を終える。  

このように、勾配降下法を用いることで誤差関数「`L(x)`」が最小となる最適な重み「`w`」を求めることができるのです。  
ちなみに、上記No1の「適当に選んだ」は、前述した**重みの初期化**を意味します。  

なお、（これまでの説明では省いていましたが）重み「`w`」を更新する際に**学習率**（Learning rate）と呼ばれる固定値を積算します。  
学習率は重み「`w`」の**更新幅を調整**する役割を持ち、通常は「`0.001`や`0.0001`」といった値を取ります。学習率が大きすぎると重みの更新幅も大きくなるため、最小値点を通り越して反対側の勾配に行ってしまう可能性があります（学習が収束しない）。逆に学習率が小さすぎると重みの更新幅も小さくなるため、学習の進捗が悪くなります（学習が終わらない）。このため、Neural Networkの学習では、学習率の値を調整しながら学習が上手くいっているのか確認する必要があります。  
