...
前節で、Neural Networkの分類精度（`y_kc`）が低いほど誤差が大きくなり、分類精度が高いほど誤差が小さくなると言いました。クロスエントロピー誤差関数の出力曲線を坂道と捉えると、**分類精度が低いほど坂道の勾配が大きく**、**分類精度が高いほど坂道の勾配が小さく**なることが分かります。分類精度が0に近いところ（かなり精度が悪い）は、断崖絶壁のような勾配になっていますね。  
