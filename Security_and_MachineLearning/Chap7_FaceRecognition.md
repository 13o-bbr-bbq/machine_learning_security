...
上図では、このノードが保持する値は「0.9378」となっているため、「このNeural Networkは、入力画像を**93.78%の確率**で**Isao Takaesu**と判断した」と見なすことができます。  

#### 7.3.1.2. Neural Networkのノード数
ところで、各層のノード数は幾つにすれば良いのでしょうか？  
これは、入力データのサイズと分類クラスの数によって変わります。  

例えば、**32x32pixelの白黒画像**を**5クラス**に分類したい場合、受け取る入力データは1024(=32x32)のベクトルとなるため、入力層のノード数は**1024個**(RGBの場合は1024x3=3072個)、出力層のノード数は**5個**となります。なお、隠れ層のノード数は、受け取るデータのサイズや特性に合わせて調整する必要がありますが、これには開発者の経験則や**グリッドサーチ**などを用いることができます(隠れ層の層数も同様に調整する)。  
