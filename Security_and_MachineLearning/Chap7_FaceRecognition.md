...
入力層から筆者の顔画像を受け取り、出力層にある上から3番目のノードが発火していることが分かります。  
上図では、このノードが保持する値は「0.9378」となっているため、「このNeural Networkは、入力画像を**93.78%の確率**で**Isao Takaesu**と判断した」と言うことができます。  

#### 7.3.1.2. Neural Networkのノード数
ところで、Neural Networkの各層のノード数は幾つにすれば良いのでしょうか？  
これは、入力データのサイズと分類クラスの数によって変わります。  

入力層のノード数は入力データのサイズによって変わります。例えば、**32x32pixelの白黒画像**を分類したい場合、受け取る入力データは1024(=32x32)のベクトルとなるため、入力層のノード数は**1024個**にします(RGBの場合は1024x3=3072個)。一方、出力層のノード数は分類クラス数によって変わります。例えば、入力データを**5クラスに分類**したい場合、出力層のノード数は**5個**にします。なお、隠れ層のノード数は、受け取るデータのサイズや特性に合わせて調整します。  
