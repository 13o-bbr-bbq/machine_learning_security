...
* 入力層の処理  
入力層は**ベクトル化した入力画像を受け取る役割**を担います。  
ところで、画像のベクトル化とは何でしょうか？画像は**pixel値の集合**で表現されます。よって、画像の**pixel値を1pixelずつ抽出**して並べていくと、ベクトルとして表現することができます（通常は各pixel値を正規化して並べる）。例えば、入力画像が**128x128pixelのRGB**の場合は、**49,152**（=128x128x3）のベクトルとなります。このように入力画像はベクトル化され、**ベクトルの各要素が入力層の各ノードに入力**されます。そして、各ノードに入力されたベクトルの要素（以下、信号）は、ネットワークで連結された**隠れ層の各ノードに伝搬**していきます。  

* 隠れ層、出力層の処理  
隠れ層は最適な出力を得るための**信号の活性化**を担います。  
隠れ層に入力された信号は**活性化**された上で、次の層に伝搬していきます。以下の図を使って活性化の様子を見ていきましょう。  
...
本例の吹き出し部分は、**隠れ層のm番目ノード**と**出力層の3番目ノード**にフォーカスし、信号がノードを伝搬する様子を表しています。  
前層から伝搬される信号を「X」、ノード間の**重み**を「W」、各ノードの**バイアス**を「b」で表しています。また、信号を活性化する関数（**活性化関数**）を「h」で表現し、隠れ層から出力される信号を「y」、出力層から出力される信号を「O」で表しています。  
 ...
 * 隠れ層の活性化  
 隠れ層のm番目ノードは、入力層から伝搬してきた信号「X10 ~ X1m」と、**入力層と隠れ層のノード間に存在**する重み「W10 ~ W1m」を**積算**して**和**を取り、これにバイアス値「b1m」を足し合わせた値を受け取ります。そして、この信号を活性化関数「h」に通した値「y1m」を次のノードに伝搬します。  

 * 出力層の活性化  
 出力層の3番目ノードは、n番目の隠れ層から伝搬していきた信号「Xn0 ~ Xnm」と、**隠れ層と出力層のノード間に存在**する重み「Wn0 ~ Wnm」を積算して和を取り、これにバイアス値「b2」を足し合わせた値を受け取ります。そして、この信号を活性化関数「h」に通した値「O2」を出力します。この出力値が、Neural Networkの「答え」となります。  
