...
とある関数「`f(x)`」の最小値を求める流れをざっくり説明すると、下図のようになります。  
 ...
 <br>
  ...
  1. 適当に選んだ点「`A`」における、関数「`f(x)`」に対する係数「`x`」の**傾き**「`⊿x`」を求める。  
  2. 点「`A`」から降下するように係数「`x`」の値を更新する（`x - ⊿x`）。その結果、点「`A'`」に移動する。  
  3. 点「`A'`」における、関数「`f(x)`」に対する係数「`x`」の傾き「`⊿x'`」を求める。  
  4. 点「`A'`」から降下するように係数「`x`」の値を更新する（`x - ⊿x'`）。その結果、点「`A''`」に移動する。  
  5. 点「`A''`」における、関数「`f(x)`」に対する係数「`x`」の傾き「`⊿x''`」を求める。 
  6. 点「`A''`」から降下するように係数「`x`」の値を更新する（`x - ⊿x''`）。その結果、点「`O`」に移動する。  
  7. 点「`O`」の傾きはほぼ0のため、この地点を最小値と判断し、係数「`x`」の更新を終える。  

イメージとしては、適当に選んだ地点を起点とし、その地点の勾配（傾き）を求め、傾きを関数の傾き方向に
それでは、適当に選んだ点「`A`」を起点にし、勾配降下法で関数の最小値を求める流れを見ていきましょう。  

勾配下降法では、点「`A`」次の式を用いてこの点Aを、関数の最小値へ近づけます。

Neural Networkの学習では、この**勾配の大きさに比例して重みの更新値を決定**します。  
