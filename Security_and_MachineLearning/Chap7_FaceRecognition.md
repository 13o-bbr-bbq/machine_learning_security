...
このように、Neural Networkに入力された入力信号は、(重みとバイアスで計算される)信号入力の総和を活性化関数で変換しながら**入力層から出力層に向かって伝搬**していきます。このことから、Neural Networkへの入力データに対する正しい出力を得るためには、各ノード間に存在する**重みの値を最適化**する必要があります。この重みを最適化することを、Neural Networkでは**学習**と呼びます(通常、バイアスは固定値)。  
