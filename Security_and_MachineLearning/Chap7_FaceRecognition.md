...
次に、この誤差を最小化するために、**勾配下降法**と呼ばれる手法を用いて**重みの値を最適化**していきます。  
...
ところで、このグラフにおける最小値は、見ての通り原点「`(0, 0)`」になりますね。とても簡単です。しかし、これは我々がグラフ全体を俯瞰（関数の形状を把握）しているから分かるのであり、Neural Networkの学習では俯瞰することはできません。  

では、グラフ全体を俯瞰できない条件の下、どのようにして最小値を求めるのでしょうか？  
そこで登場するのが勾配降下法です。  

勾配降下法を一言でいうと、「**関数の傾きを利用**して、その関数の最小値を求める方法」です。  
...
このように、「勾配を求める⇒係数`x`を更新する」という単純な計算を勾配がほぼ0（関数「`f(x)`」の最小値）になるまで繰り返します。  

では、どのようにしてNeural Networkの重みを勾配降下法で最適化するのでしょうか？  
前節で述べたように、Neural Networkの学習は「**誤差関数が最小になるように重みを最適化する**」ことでした。つまり、上図の**とある関数「`f(x)`」を誤差関数「`L`」**、**係数「`x`」を重み「`w`」**に置き換えて勾配降下法を用いれば、誤差関数が最小値になる重みを求めることができます。  

以下に、勾配降下法で最適な重みを求めるイメージ図を示します。  
 ...
 <br>
  ...
  1. 適当に選んだ点「`A`」における、関数「`L(w)`」に対する重み「`w`」の**勾配**「`⊿w`」を求める。  
  2. 勾配方向に重み「`w`」の値を更新する（`w - ⊿w`）。その結果、点「`A'`」に移動する。  
  3. 点「`A'`」における、関数「`L(w)`」に対する重み「`w`」の勾配「`⊿w'`」を求める。  
  4. 勾配方向に重み「`w`」の値を更新する（`w - ⊿w'`）。その結果、点「`A''`」に移動する。  
  5. 点「`A''`」における、関数「`L(w)`」に対する重み「`w`」の傾き「`⊿w''`」を求める。 
  6. 勾配方向に重み「`w`」の値を更新する（`w - ⊿w''`）。その結果、点「`O`」に移動する。  
  7. 点「`O`」の勾配は**ほぼ0**のため、この地点を**最小値**と判断し、重み「`w`」の更新を終える。  

このように、先程のとある関数「`f(x)`」の例と同じく、誤差関数「`L(x)`」が最小となる重み「`w`」を求めることができるのです。  

なお、（これまでの説明では省いていましたが）重み「`w`」を更新する際に**学習率**（Learning rate）と呼ばれる固定値を積算します。学習率は重み「`w`」の**更新幅を調整**する役割を持ち、「`0.001`や`0.0001`」といった値を取ります。学習率が大きすぎる場合は更新幅も大きくなるため、最小値点を通り越して反対側の勾配に行ってしまいます（学習が収束しない）。逆に学習率が小さすぎる場合は更新幅も小さくなるため、学習の進みが悪くなります（学習が終わらない）。このため、学習率の値を調整しながらNeural Networkの学習が上手くいっているのか確認する必要があります。  
