...
「機械学習なのに傾き？」と疑問に思う方もいると思いますが、前節でも登場したクロスエントロピー誤差関数のグラフを見れば一目瞭然です。  

 <div align="center">
 <figure>
 <img src='./img/7_error_func_cross.png' alt='Cross entropy error function' width=400><br>
 <figcaption>クロスエントロピー誤差関数の出力（再登場）</figcaption><br>
 <br>
 </figure>
 </div>

前節で、Neural Networkの分類精度（`y_kc`）が低いほど誤差が大きくなり、分類精度が高いほど誤差が小さくなると言いました。クロスエントロピー誤差関数の出力曲線を坂道と捉えると、**分類精度が低いほど坂道の勾配が大きく**、**分類精度が高いほど坂道の勾配が小さく**なることが分かります。  
