...
ここでは、赤い破線で囲った「隠れ層1の1番目ノードから隠れ層2の1番目ノードの間にある重み「`w1_11`」」を更新する流れに沿って誤差逆伝搬法を説明することにします。つまり、`w1_11`で誤差関数「`E`」を偏微分して勾配を求めることを目標とします。勾配が計算できれば、勾配降下法で`w1_11`を最適化できますね。  

しかし、ここで問題が発生します。上図を見ても分かる通り、`E`は`w1_11`の関数ではないため、直接偏微分することはできません。`w1_11`で`E`を偏微分するためには、何らかの関数で`W1_11`を`O`に変換し、`O`で`E`を偏微分する必要があります。  
