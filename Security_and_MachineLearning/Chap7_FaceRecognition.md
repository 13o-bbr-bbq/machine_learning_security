...
Neural Networkは**層**と**ノード**から構成される**ネットワーク**であり、**ベクトル化**された入力データを**入力層**から受け取り、分類結果を**出力層**から出力します。入力層と出力層の間には1つ以上の**隠れ層**が存在し（隠れ層は**中間層**とも呼ばれる）、Neural Networkの**表現力**を高める役割を持ちます（表現力の詳細は後述します）。  
...
| 重み（weight）|
|:--------------------------|
| Neural Networkの分類精度に大きな影響を与えるパラメータ。Neural Networkの**学習**では、学習データ（データとクラス（答え））を基に、**データに対応する「クラスが紐づけられた出力ノード」に信号が集まるように重みを繰り返し更新**し、重みの値を最適化する。重みが最適化されると、入力データに対応する出力ノードが最も活性化されるように信号が伝搬されるようになる。なお、重みは学習前に**初期化**する必要があり、**活性化関数**の種類に応じて最適な初期化方法を採用する。代表的な初期化方法として、「Xavierの初期値」や「Heの初期値」などがある。なお、単純にゼロ初期化をすると、上手く学習できないことが知られている。|

| バイアス（bias）|
|:--------------------------|
| ノードへの入力信号値を微調整するためのパラメータ。重みと同様に学習することで最適化される。|
...
隠れ層のm番目ノードは、入力層から伝搬してきた信号「X10 ~ X1m」と、（入力層と隠れ層の）ノード間にある重み「W10 ~ W1m」を**積算**して**和**を取り、これにバイアス値「b1m」を足し合わせた値を受け取ります（この様子は、吹き出しの下にある数式で表しています）。そして、この**信号を活性化関数「h」で整えて**ノードの出力値「y1m」とし、次のノードに伝搬させます。  
 ...
 * ランプ関数（ReLU）  
 ...
 ReLUは隠れ層を増やしても勾配消失問題が起きにくい（Neural Networkの学習が安定する）ため、現在ではよく使われています。  
 ...
 * 恒等関数（Identity function）  
 ...
 恒等関数は出力層の活性化を省略したのと同じ挙動になるため、回帰問題（入力に対する出力を予測するなど）に使われます。  

 * ソフトマックス関数（softmax function）  
 ソフトマックス関数はNeural Networkの**出力層で使用される活性化関数**です。  
 以下のように、**入力値（Input）に対する出力値（Output）の合計を1**にします。  
 ...
 上記のように、出力層の5つのノード「`node[0], node[1], node[2], node[3], node[4]`」がそれぞれ「`5, 2, 3, 4, 1`」の値を保持していたとします。これらの値をソフトマックス関数に入力すると、それぞれ「`0.6364086465588309, 0.031684920796124276, 0.08612854443626872,0.23412165725273665, 0.011656230956039607`」を出力します。この出力値を見ると、「入力値に応じて出力値が変化」し、「出力値の合計値は1」になることが分かります。  
 このように、ソフトマックス関数は**複数クラスの分類**に適しています（最も高い出力値を持つノードをNeural Networkの答えとする）。また、出力値の合計値が1になることから、**各出力値は分類確率**と見なすこともできます。  
