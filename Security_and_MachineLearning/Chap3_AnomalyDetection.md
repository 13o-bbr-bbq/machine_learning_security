...
ポート`8888`で接続を待ち受け、クライアントから送信されるパラメータ「`param`」の値（`normal` or `attack` or `load`）に応じてレスポンス応答時間を調整します。例えば、「`param=normal`」の場合は「`0.01`(sec)」waitした後に応答、「`param=attack`」の場合は「`0.5`(sec)」waitした後に応答します。また、「`param=load`」の場合は、リクエストを受信する度に「`0.01`(sec)」ずつwaitの時間を長くしていきます。  

このようにwait時間を調整することで、**突然のレスポンス低下**や**緩やかなレスポンス低下**の事象を再現しています。  
...
第1カラムはタイムスタンプ、第2カラムはログレベル、第3カラムは検証用サーバの応答時間（sec）です。  

##### 実行結果: 突然のレスポンス低下の検知
検証用クライアントを「`REQ_MODE=rapidly`」で起動します。  
上述した通り、このモードでは「100リクエスト目までは正常応答（`0.01`(sec)）」、101リクエスト以降は「`0.5`(sec)」で応答）となります。  
このように、意図的に異常部位を作り出すことで**突然のレスポンス低下**を再現します。  

検証用クライアントが出力する`server_response.csv`には以下のようなログが記録されていきます。  
...
上記の通り、100リクエスト目を超えた`2018-12-30 07:30:37,692`あたりから**急激に応答時間が遅くなっている**ことが分かります。  
この時、異常検知システムは以下のログを標準出力します。  

```
ERROR: Detact anomaly!! Now date: 07:30:47.355
```

これまでの部分時系列と距離が離れているため、異常検知システムは正しく**異常を検知**しています。   
なお、今回は`WINDOW_SIZE = 10`としているため、応答時間低下の発生から少し時間を置いて異常検知します。  

##### 実行結果: 緩やかなレスポンス低下の検知
検証用クライアントを「`REQ_MODE=slowly`」で起動します。  
上述した通り、このモードでは「100リクエスト目までは正常応答（`0.01`(sec)）」、101リクエスト以降は「`0.01`(sec)」ずつ応答時間が遅くなっていきます。  
このように、意図的に変化点を作り出すことで**緩やかなレスポンス低下**を再現します。  
なお、「緩やかなレスポンス低下」は「急激なレスポンス低下」と比べて、正常と異常の部分時系列の距離が小さくなることが予想されます。  
よって、ここでは`anomaly_detection.py`の異常レートを以下のように設定します。  
...
上記の通り、100リクエスト目を超えた`2018-12-30 08:21:16,195`あたりから**徐々に応答時間が遅くなっている**ことが分かります。  
この時、異常検知システムは以下のログを標準出力します。  
