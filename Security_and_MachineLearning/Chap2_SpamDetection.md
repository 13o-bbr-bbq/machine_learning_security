...
これは、私が**過去に見聞き（学習）した知識**を基に、テキストに含まれる「認識、脳内、信号、ニューロン、発火、伝播、判断」等の単語から「脳神経学！」と判断したからです。ところで、このテキストにはラーメンに関する単語も含まれています。例えば「味噌、ラーメン、白味噌、軒」等です。なぜ私は「ラーメン！」と判断しなかったのでしょうか。それは、このテキストにはラーメンに関する単語よりも脳神経学に関する単語の方が**出現頻度が高い**からです。つまり、単一の単語のみを見るだけではなく、テキストを構成する全単語を**総合的に見て**判断しています。  

ナイーブベイズも同様のロジックでテキスト分類を行います。  
ナイーブベイズは教師あり学習モデルです。よって、上述の「**過去に見聞き（学習）した知識**」に相当する学習データを準備する必要があります。ナイーブベイズでは学習データを**カテゴリテーブル**と呼ばれるデータセットで表現します。  
...
カテゴリと各カテゴリを構成する単語を紐付けただけの非常にシンプルな構造です。  
