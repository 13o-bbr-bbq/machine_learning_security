...
## 2.0. 目的
本ブログは、**スパム検知システムの実装**を通して、**テキスト分類アルゴリズム**の**ナイーブベイズ**を理解することを目的としています。  

## 2.1. スパム検知
**スパム検知**（Spam Detection）とは、受信者の意向を無視して無差別かつ大量にばら撒かれる**スパムメールを検知する機能**であり、メジャーなメーラーには標準で実装されています。  

### 2.1.1. 本ブログのコンセプト
スパム検知システムを実装するためには、受信メールのタイトルや本文から**スパムメールの特徴を抽出**し、これを手掛かりにメールを**正常またはスパムに分類**すれば良いと考えられます。また、可能であれば、分類時にそのパーセンテージ（確率）も併せて計算できた方が運用上は好ましいと言えます。なぜならば、メールには機械で白黒つけ難い微妙なメールが存在するからです。このような場合は、「分類確率が70%以下の場合は人間の判断を仰ぐ」のようなルールを決めておくことで、誤検知を減らすことができます。  

本ブログでは、上記コンセプトのスパム検知システムを**ナイーブベイズ**で実装していきます。  

## 2.2. ナイーブベイズ（Naive Bayes）
ナイーブベイズは、**ベイズの定理**を使用して**分類問題**を解く機械学習アルゴリズムであり、**文書分類**等に利用することが可能です。**実装が容易**かつ**処理速度**も早いため様々な分野で利用されており、本ブログで解説するスパム検知の他に、ブログやニュース記事の自動ジャンル分けや[Webページの種別判定](https://www.mbsd.jp/blog/20160113.html)等で利用されています。  
...
### 2.2.1. ナイーブベイズ入門の入門
ここでいきなり質問ですが、以下の文書を「ラーメン」または「脳神経学」のカテゴリに分類するとしたら、皆さんはどちらに分類するでしょうか？  
...
私は「脳神経学」に分類します。  
これは、私が**過去に見聞き（学習）した知識**を基に、文書に含まれる「`認識`、`脳内`、`ニューロン`、`発火`」等の単語から「脳神経学！」と判断したからです。ところで、この文書にはラーメンに関する単語も含まれています。例えば「`味噌`、`ラーメン`、`軒`」等です。ではなぜ私は「ラーメン！」と判断しなかったのでしょうか。それは、この文書においては、ラーメンに関する単語よりも**脳神経学に関する単語の出現頻度が高い**からです。つまり、単一の単語をマッチングして判断したのではなく、文書に含まれる各単語を**総合的に観察**しています。この時、文書の**文脈は考慮していません**。  
...
ナイーブベイズは**教師あり学習**モデルです。よって、上述した「**過去に見聞き（学習）した知識**」に相当する学習データを準備する必要があります。教師あり学習ですので、**学習データの良し悪しがナイーブベイズの分類精度を左右**します。  

ナイーブベイズでは学習データを**カテゴリテーブル**と呼ばれるデータセットで表現します。  
...
このテーブルは、分類の肝となる以下2つの計算を行うために利用されます。  
 ...
 1. カテゴリの出現確率  
 **「カテゴリテーブルの単語総数」の内、「各カテゴリの単語数」が占める割合を計算**をして求める。  
 2. 単語の出現確率  
 **「文書に含まれる単語」が「各カテゴリ」に幾つ含まれているのかカウント**して求める。  

上記2つの確率をカテゴリ毎に積算し、**計算結果が大きい**カテゴリに分類します。  
このように、ナイーブベイズは非常に単純な計算のみで文書分類を行います。  
...
いい感じに文書が単語単位に分割されました。  

### 2.2.4. 不要な単語の除去
分割された各単語を眺めてみると、「私」「が」「を」「、」「することができる」等のような単語が含まれていることが分かります。これらは**どのような文書でも頻繁に出現する「ありふれた単語」**であるため、**文書分類には貢献しません**。このような分類に貢献しない単語を**ストップワード**と呼びます。  

| ストップワード（Stop Word）|
|:--------------------------|
| 自然言語処理の精度向上のため、処理対象から除外せざるを得ない単語。日本語では「ここ」「は」「はい」「これら」など、英語では「is」「a」「this」「the」等のありふれた、**文書の特徴付けに貢献しない単語や記号**等。|

文書分類の精度を上げるためには、文書からストップワードを除去する必要があります。  
...
### 2.2.5. カテゴリの出現確率の計算
上述したようにカテゴリの出現確率は、**「カテゴリテーブルの単語総数」の内、「各カテゴリの単語数」が占める割合を計算**して求めることができます。  
よって、単純に以下のように計算できます。  
...
**カテゴリ毎に各単語の出現確率を積算**し、これに**各カテゴリの出現確率との積**を取れば**文書が各カテゴリに属する確率**が求まります。  
...
### 2.2.7. ラプラススムージング
ゼロ頻度問題を克服するために、**ラプラススムージング**と呼ばれるテクニックが利用できます。  
...
今回は、ラプラススムージングとして、単語の出現回数（分子の数値）に1を加えます。  
...
次に、カテゴリ内の各単語出現確率を積算し、各カテゴリにおける単語の出現確率を求めます。  
...
### 2.2.8. 分類
最後に、各カテゴリにおける単語の出現確率とカテゴリの出現確率を積算し、文書が各カテゴリに属する確率を求めます。  
...
上記の結果から、「脳神経学（0.000000000012582912）> ラーメン（0.000000000006291456）」のため、この文書はカテゴリ「脳神経学」に分類されることになります。  

### 【参考情報】アンダーフロー対策
上記の手順により、ナイーブベイズで文書分類できることが分かりました。  
ここで、カテゴリ出現率は「0.000000000012582912」や「0.000000000006291456」のように**非常に小さな値**になることが分かります。上記の例では単語数の少ないカテゴリテーブルを使用しましたが、実戦では多数の単語が含まれるカテゴリテーブルを使うことになります。よって、単語の出現確率を求める際の分母が大きくなることで、**アンダーフロー**が発生する可能性が高くなります。  
...
ここで、対数には「対数は**元の値より大きくなる**」「元値の乗算の大小は**対数の加算の大小と同じ**」の特性があります。  
...
なお、Pythonでは、`math.log(元値)`メソッドで対数を求める事ができます。例えば、以下のようなアンダーフローが発生する積算を、対数の加算に置き換えることで、アンダーフローを回避することができます。  
...
ちなみに、本メールは筆者がとあるサービスで会員登録した際に実際に受信したメールを基にしています（もちろんスパムではありません）。  
