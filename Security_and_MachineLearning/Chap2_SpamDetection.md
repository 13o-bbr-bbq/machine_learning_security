...
### 2.1. ナイーブベイズ
ナイーブベイズは**テキスト分類**を行う機械学習アルゴリズムであり、分類対象のテキストを予め定義した**カテゴリ**（=クラス）に分類することができます。ナイーブベイズは、本ブログで説明するスパム検知の他に、ニュース記事やブログのジャンル分け（政治、スポーツ、科学技術、エンタメ等）、[Webページの種別判定](https://www.mbsd.jp/blog/20160113.html)等に使用されています。  

##### ナイーブベイズ入門
いきなり質問ですが、以下のテキストを「ラーメン」または「脳神経学」のカテゴリに分類するとしたら、あなたはどちらのカテゴリに分類するでしょうか？  
...
私の場合は「脳神経学」に分類しました（それは正解です）。  
これは、私が**過去に見聞き（学習）した知識**を基に、テキストに含まれる「認識、脳内、信号、ニューロン、発火、伝播、判断」等の単語から「脳神経学！」と判断したからです。ところで、このテキストにはラーメンに関する単語も含まれています。例えば「味噌、ラーメン、白味噌、軒」等です。なぜ私は「ラーメン！」と判断しなかったのでしょうか。それは、このテキストにはラーメンに関する単語よりも脳神経学に関する単語の方が**出現頻度が高い**ためであり、これにより「脳神経学」の方に分類しています。つまり、単一の単語だけを見るだけではなく、テキスト全体の構成単語を**総合的に見て**判断しています。  

ナイーブベイズも同様のロジックでテキスト分類を行います。  
ナイーブベイズは教師あり学習モデルとなるため、上述の「**過去に見聞き（学習）した知識**」に相当する情報を学習データとして準備する必要があります。ナイーブベイズでは学習データを**カテゴリテーブル**と呼ばれるデータセットで表現します。  
...
カテゴリと各カテゴリを構成する単語を紐付けただけのシンプルな構造です。  
...
|※|ちなみに|、|この|画像|は|「○○|軒|」|の|白味噌|ラーメン|。|濃厚|かつ|マイルド|な|味|わいで|美味|しい|。
```
