...
多くの方は「脳神経学」に分類するのではないでしょうか。それは正解です。  
これは、**過去に見聞きした知識**を基に、テキストに含まれる「認識、脳内、信号、ニューロン、発火、伝播、判断」等の**単語の出現頻度**を基に「脳神経学だ！」と判断しているからだと思います。ところで、このテキストにはラーメンに関する単語も含まれています。例えば「味噌、ラーメン、白味噌、軒」等です。しかし、ラーメンに関する単語よりも脳神経学に関する単語の方が**出現頻度が高い**ため、テキスト全体を**総合的に見て**「脳神経学」に分類しています。  

このように、**文脈を考慮せず**とも、**単語の出現頻度**だけでも正しくカテゴリ分類することができます。ナイーブベイズは、これと同じような分類ロジックを実現します。  

ナイーブベイズでは、上述の「過去に見聞きした知識」に相当する情報を**カテゴリテーブル**として表現します。  
以下は、
