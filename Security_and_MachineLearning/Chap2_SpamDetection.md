...
ナイーブベイズは、**ベイズの定理**を使用して**分類問題**を解く機械学習アルゴリズムであり、**文書分類**等に利用することが可能です。ナイーブベイズは**実装が容易**かつ**処理速度**も早いため様々な分野で利用されており、本ブログで解説するスパム検知の他に、ブログやニュース記事の自動ジャンル分け（政治、スポーツ、科学技術、エンタメ等のカテゴリに分ける）や[Webページの種別判定](https://www.mbsd.jp/blog/20160113.html)（HTTPレスポンス内容からログイン、会員登録、問い合わせフォーム等の種別に分ける）等で利用されています。  
